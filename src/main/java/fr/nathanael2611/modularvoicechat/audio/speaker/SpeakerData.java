...
public class SpeakerData implements NoExceptionCloseable
{
    ...
    public byte[] write(int id, byte[] array, int volumePercent)
    {
        if (isAvailable(id))
        {
            ...
            {
                PlayerEntity speaker = (PlayerEntity) Minecraft.getInstance().world.getEntityByID(id);
                if (speaker != null)
                {
                    try
                    {
                        double atan2 = Math.atan2(speaker.posZ - Minecraft.getInstance().player.posZ, speaker.posX - Minecraft.getInstance().player.posX);
                        double angle = Math.toRadians(Minecraft.getInstance().player.rotationYawHead) - atan2;
